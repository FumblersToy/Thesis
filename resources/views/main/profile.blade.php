                        ...
                        <div class="flex-1 text-center md:text-left">
                            ...
                            <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 mb-6">
                                ...
                                <div class="flex gap-3 justify-center md:justify-start">
                                    ...
                                    @endif
                                    @if($user && $currentUserBusiness && $musician && $user->id != $profileUserId)
                                        <button id="ratingButton" class="bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-500 hover:to-orange-600 px-6 py-2 rounded-xl text-white text-sm font-medium transition-all duration-300 shadow-lg hover:shadow-xl border-2 border-white/20" data-musician-id="{{ $profileUserId }}">
                                            ⭐ Rate Musician
                                        </button>
                                    @endif
                                </div>
                            ...
                            <div class="flex justify-center md:justify-start gap-8 md:gap-10 mb-6">
                                ...
                                @if($musician)
                                <div class="text-center md:text-left" id="ratingDisplay">
                                    <span class="font-bold text-gray-800 text-lg" id="avgRating">0.0</span>
                                    <span class="text-yellow-500 ml-1">⭐</span>
                                    <span class="text-gray-600 ml-1">(<span id="ratingCount">0</span> ratings)</span>
                                </div>
                                @endif
                            </div>
                            ...
                            <div class="text-left space-y-2">
                                ...
                                @php
                                    $socialLinks = [];
                                    if ($musician) {
                                        if ($musician->instagram_url) $socialLinks['instagram'] = $musician->instagram_url;
                                        if ($musician->facebook_url) $socialLinks['facebook'] = $musician->facebook_url;
                                        if ($musician->twitter_url) $socialLinks['twitter'] = $musician->twitter_url;
                                        if ($musician->youtube_url) $socialLinks['youtube'] = $musician->youtube_url;
                                    } elseif ($business) {
                                        if ($business->instagram_url) $socialLinks['instagram'] = $business->instagram_url;
                                        if ($business->facebook_url) $socialLinks['facebook'] = $business->facebook_url;
                                        if ($business->twitter_url) $socialLinks['twitter'] = $business->twitter_url;
                                        if ($business->website_url) $socialLinks['website'] = $business->website_url;
                                    }
                                @endphp
                                ...
                                <div class="flex items-center gap-3 mt-4">
                                    @foreach($socialLinks as $platform => $url)
                                        <a href="{{ $url }}" target="_blank" rel="noopener noreferrer" class="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-r from-purple-100 to-pink-100 hover:from-purple-200 hover:to-pink-200 transition-all duration-300 group">
                                            @if($platform === 'instagram')
                                                <svg class="w-5 h-5 text-purple-600 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                                            ...
                                            @elseif($platform === 'twitter')
                                                <svg class="w-5 h-5 text-sky-500 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>
                                            @elseif($platform === 'youtube')
                                                <svg class="w-5 h-5 text-red-600 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                                            @elseif($platform === 'website')
                                                <svg class="w-5 h-5 text-gray-700 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>
                                            @endif
                                        </a>
                                    @endforeach
                                </div>
                    ...
                    <article class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 animate-scale-in border border-gray-200">
                        ...
                        >
                            <div class="flex items-center gap-4 mb-4">
                                ...
                                @if($isOwner)
                                    <button class="delete-post-btn text-red-500 hover:text-red-700 transition-colors p-2" 
                                            data-post-id="{{ $post->id }}" 
                                            title="Delete post">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (ratingButton) {
                ratingButton.addEventListener('click', function() {
                    ...
                    showRatingModal(musicianId);
                });
            }

            // Load average rating on page load if musician profile
            @if($musician)
            loadAverageRating({{ $profileUserId }});
            @endif
                                    ...
                                    <div class="flex items-center gap-4 mb-4">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <a href="/profile/${postData.userId}" class="font-bold text-gray-800 text-xl hover:text-purple-600 transition-colors" onclick="event.stopPropagation();">${postData.userName}</a>
